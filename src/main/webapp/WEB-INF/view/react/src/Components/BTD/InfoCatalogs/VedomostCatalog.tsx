...
class VedomostCatalog extends React.Component<Props,State,any> {
  ...
  saveDocument = () => { 
    ...
    if(!this.state.isNew) {
      ...
      result = axios({
        method: 'post',
        url: process.env.PUBLIC_URL+'/vedomost/update',
        params: {
            name: this.state.editedName,
            id: this.state.editeId
        }
      ...
      <Dialog
        ...
        >
          <DialogContent>
            <table>
              ...
              <tr>
                  <td>
                    <div>
                      <h3 style={{ display: 'inline-block', color:'#4c5b65', height:'20px', fontSize: '11pt', paddingLeft: '15px'}}>Номер ведомости:</h3>
                        <TextField
          ...
          <DialogActions>
            { this.state.isNew
                ? <div></div>
                : <Button  onClick={this.deleteDocument} className={classes.root}>
                    <DeleteIcon />
                  </Button>
            } 
...
export default withStyles(styles)(VedomostCatalog);